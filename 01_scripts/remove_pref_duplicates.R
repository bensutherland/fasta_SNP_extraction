...
for(i in 1:nrow(corresp_data)){
  ...
  if(corresp_data$dropped.priority1[i]=="yes" & corresp_data$retained.priority1[i]=="no"){
    ...
    # Switch the dropped to the retained
    corresp_data$marker_to_drop[i] <- corresp_data$mname.retained[i]
    
    # Reporting
    corresp_data$switched[i] <- "true-keep_pri1"
  
  # If the marker dropped is priorty one, and the retained marker is priority one, do nothing  
  ...
  } else if(corresp_data$dropped.priority1[i]=="no" & corresp_data$dropped.priority2[i]=="yes"){
...
#  remove it from the drop list and let the duplicate screen remove the duplicate
dim(corresp_data)
